...
不过只开启转发功能还是不足以让局域网的设备连接上互联网。原因在于局域网使用的是私有地址段（10.x.x.x、192.168.x.x 等）：同一个私有地址同时存在于千千万万个家庭网络里，互联网上的路由器不可能为它建立回程路由——应答根本不知道该送回哪一家。所以按照约定，运营商见到源地址为私有地址的数据包会直接丢弃。就算我们把请求转发了出去，它也走不出第一跳。所以我们需要上面提到的Masquerading功能。
