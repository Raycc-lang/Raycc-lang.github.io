...
title:  "(五) 自动微分"
description: "《The Little Learner》系列第五篇，讲解计算图与自动微分的前向反向模式，并用Python闭包实现支持反向传播的自动微分引擎"
...
再看一个嵌套函数的情况。比如，我们有一个先算 $g$ 再算 $f$ 的复合函数，写成 $f(g(a))$，那我们怎么知道这个复杂函数对最初的 a 的梯度是多少呢？
...
把整个过程和中间结果都记录下来，会得到一个**有向无环图（Directed Acyclic Graph, DAG）**，这个结构便是计算图。之所以称其为图而非树，是因为同一个中间结果可以被多处复用（如下文 $\sin(x^2+3x)$ 里的 $x$ 同时参与 $x^2$ 和 $3x$，两条边在 $+$ 处汇合），而反向模式必须把沿不同路径传回的梯度相加。本例中没有分支，它退化为简单的链式结构：$x→v_1
 →v_2
 →v_3$。
...
还是上面的例子，反向模式从输出开始反向传播“影响”。注意这里要换一个记号：正向模式里 $\dot{v}$ 表示“$v$ 对**输入** $x$ 的导数”；而反向模式里我们追踪的是“**输出**对 $v$ 的导数”，习惯记作 $\bar{v}$。两者方向恰好相反，如果混用同一个符号，很容易在这里晕头转向——我自己初学时就在这儿栽过跟头。
...
$$
然后根据链式法则，向上游传播：
- $v_2$ 的梯度为：$\bar{v}_2 = \bar{v}_3 \cdot \frac{\partial v_3}{\partial v_2}$
- $v_1$ 的梯度为：$\bar{v}_1 = \bar{v}_2 \cdot \frac{\partial v_2}{\partial v_1}$
- 输入 $x$ 的梯度为：$\bar{x} = \bar{v}_1 \cdot \frac{\partial v_1}{\partial x}$
...
- 梯度：$\frac{df}{dx}\big\vert_{x=2} \approx -5.873$
...
**最终结果：** $\frac{df}{dx}\big\vert_{x=2} \approx -5.873$
...
link 函数的三个参数里，真正驱动传播的是 z——从下游传回来的梯度；grad_tape 是一个所有节点**共享**的字典，各个叶节点的梯度最终都汇集在这里，这也是反向传播结束后我们提取结果的地方（至于 _result_dual，反向传播中通常用不到，保留它只是为了让所有 link 函数的接口保持一致）。

link函数执行两个关键步骤：
(1) 根据当前运算的梯度规则计算中间梯度；
(2) 递归调用输入变量的link函数，继续向计算图上游传播梯度。

注意，link 本身并不往 grad_tape 里写梯度，它只负责把梯度往上送。梯度的真正汇集发生在叶节点处（由下文 end_of_chain 用累加完成）--这样同一个叶节点经多条路径传回的梯度才能相加，而不是被后到的分支覆盖。
...
注意 end_of_chain 里一个此前没有出现过的细节：它写的是 `dg + z`，是**累加**而不是赋值。这正是多路径情形的落点——同一个叶节点可能通过多条路径影响输出（就像手算例子里，$x$ 同时通过 $x^2$ 和 $3x$ 两条路径影响结果），每条路径传回来的梯度都要加在一起，而不是互相覆盖。手算例子最后那步 $-3.356 + (-2.517)$，在代码里就发生在这一行。
...
def prim2(primal_fn: Callable, derivative_fn: Callable) -> Callable:
    ...
    def primitive(da, db):
        ...
        def link(d, g, grad_tape) -> None:
            ...
            # 不写 tape：中间节点的梯度无需保存，叶节点处的累加交给 end_of_chain
            ka, kb = get_k(da), get_k(db)
...
def get_grad(y: Dual, wrt: Theta) -> Theta:
    ...
    sigma = {}               # 梯度字典（id到梯度的映射）。注意：计算图本身藏在闭包的调用链里，这个字典只负责收集各叶节点的梯度结果
...
学完自动微分的进阶实现，我们立刻就会正式踏入神经网络的世界。届时，我们将用自己亲手打造的自动微分引擎，来训练真正的神经网络模型。敬请期待！