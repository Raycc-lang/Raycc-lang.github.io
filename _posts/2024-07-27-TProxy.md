...
description: "讲解透明代理原理，分析iptables REDIRECT方案的缺陷，并用nftables配合Linux TProxy内核模块实现真正可用的透明代理"
...
然而，多数人只知其名，却不解其运作机制。甚至常年使用透明代理的人，也未必说得清 UDP 流量为什么总是配不对。本文先讲清透明代理的概念，再走一遍传统的 REDIRECT 方案，看看它在哪里露出破绽；正是这个破绽，解释了 Linux 内核为什么专门提供一个 TProxy 模块，也解释了本文最后那两段配置里每一行的用意。
...
1. 找出需要代理的数据包（需要了解Netfilter,才能在正确的地方找到需要数据包，不过本文暂不涉及）； 
2. 把这些数据包转发到代理服务器即可。
...
准备好这些，接下来我们尝试使用TProxy的方式实现前面例子。
...
虽然上面这么大一坨，看着很复杂，最关键的就这一句```udp dport 53 meta mark set 1```，它只做了一件非常简单的事儿，翻译成人话就是“把去往53端口的UDP流量做上标记‘1’。” 目的是跟其他的流量加以区分。（其他部分是关于Netfilter和Nftables的语法，这些可以自行了解。）
...
先补充一个重要的背景知识：Linux中有0-255，共256张路由表。其中0，253，254，255号路由表是特殊路由表，默认已经被配置好了别名。如果不指定使用哪一张表就会默认使用254号表，也叫main。可以用```cat /etc/iproute2/rt_tables```命令查看表的别名, 也可以修改这个文件，给自定义的表添加一个别名。

上面的语句首先增加一个策略把所有被标记过的数据包都送到100号路由表。（为什么网上所有的教程都说把标记过的流量放到100号表，你应该能猜到100号并没有啥特别的，就是简单好记，Linux内核文档说明中也使用100号路由表作为例子。）  

然后我们给100号路由表添加一条路由规则。观察第二条命令 ```local 0.0.0.0/0  dev lo```, 这条命令告诉内核这些所有的地址都属于本地地址，通过lo接口处理这些数据。

这样做，我们就让原本要离开的数据包重新进入本地了。

为什么这一步"很关键"？因为这些数据包的目标地址本来就不是本机。如果按默认路由表处理，内核查完路由就会把它们直接转发出去，后面的 TProxy 规则根本没有机会碰到它们。只有先用策略路由把它们"骗"进本地协议栈，"递交给本机的代理进程"这件事才有可能发生。

接下来我们把标记过的数据挑出来，然后再通过TProxy模块导入到代理服务器，脚本如下：
...
上面语句的意思是：“把去往53端口、带有‘1’标记的udp流量，递交给1080端口上的代理进程”。

注意这里的用词是"递交"而不是"转发"。`tproxy to :1080` 不像 REDIRECT 那样改写数据包的目标地址；数据包原封不动，只是被直接递交给本机 1080 端口上监听的代理。代理进程通过设置了 IP_TRANSPARENT 选项的套接字收下这个包，再从包头里读出它本来要去的地址。

现在回想 REDIRECT 的死穴：目标地址一旦被改写，UDP 就永远失去了"我要去哪"这个信息。TProxy 的全部价值就在这一处不同——**地址原封不动，信息一点不丢**。这就是内核要为透明代理专门做一个模块的原因，也是前面第4条要求"代理服务器必须支持TProxy"的含义：代理得会用这种特殊的套接字。
