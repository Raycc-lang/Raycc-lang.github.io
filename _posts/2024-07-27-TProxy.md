...
透明代理是一个既实用又有趣的概念。对于经常使用代理的人来说，它像一位“隐形的流量向导”——客户端无需任何配置，就能自动将数据包导向目标服务器。这种特性让透明代理成为企业网络监控、家庭家长控制等场景的利器。然而，多数人只知其名，却不解其运作机制：它如何悄无声息地拦截并转发流量？
本文会讲解这个概念，然后在Linux上实现一个最简单的透明代理。
