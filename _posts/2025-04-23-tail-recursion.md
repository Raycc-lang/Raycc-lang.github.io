...
= 120
```
把5带入程序后首先得到5 * factorial(5 - 1)，我们可以计算出 5 - 1的值4， 但是我们还不知道factorial(4)的值，因此先保留5 *这个操作，具体来说是保存在一个叫做堆栈（Stack，后面简称栈，不要和堆 Heap弄混了）的数据结构中，等得出factorial(4)的值之后再执行运算。可以看到它是一个先递进再回归的过程，这种过程被称为递归过程。
...
是的，你没看错！上面这个函数是一个递归函数，这个递归函数求值的过程却是一个线性迭代过程。需注意这是求值过程的概念模型（也是支持尾递归优化的语言中的实际行为，在Python里，这个函数的每次调用仍然会压栈。
...
我们之前说过，在递归过程中运算和值会先保存在栈里：factorial(5) 挂起了 4 次待完成的乘法，factorial(10000) 就要挂起 9999 个栈帧——内存需求随递归深度线性增长，这就是问题所在。如果把这个递归函数写成尾递归的形式，在计算的过程中，状态会随着参数传递给下一层，运算结果不在需要用到之前的运算状态。编程语言实现时若对递归过程进行优化，使其直接丢弃前序运算而非压栈保存，可显著降低函数的内存需求。

所以，递归函数未必消耗大量资源，前提是编程语言实现了尾递归优化。

尾递归优化听起来很美好，在SICP写作的年代，Scheme是唯一明确要求尾调用优化的语言；如今Lua的语言定义也做了同样的保证，ES6规范甚至也要求了它，虽然几乎没有引擎真正实现。

常常可以看到scheme的使用者，在函数定义子函数，然后调用子函数作为返回值。然而在Python以下写法会导致额外一层栈调用：
```python
def factorial(n):
...
1. 实现尾递归优化以后，会破坏栈跟踪（Stack Trace）的完整性，增加调试难度。
...
所以我们使用Python这个工具的时候，就得适应这个工具的特点，记住典型的Python实现支持1000层的栈调用（可以通过sys.setrecursionlimit设置），达到或者超过它会爆栈。

关于"什么时候用递归、什么时候用循环"，网上的说法五花八门，但大多只给结论、不给判断依据。其实前文的分析已经给出了一个可靠得多的标准：**看数据的形状**。在之前的博客中我讲到一个观点是一个设计良好的程序应该能反应它所操作的数据的数据结构，而有的数据结构它的定义中就指向自身，它天然就是递归的。最典型的递归数据结构就是各种树了，因此把遍历树的方式写成递归的方式是最直观最清楚的。

吉多·范罗苏姆也在他的博文中提到树的遍历，他认为1000层栈够用了，并且在结尾写道尾递归的形式很容易改写成常规的迭代循环的方式。所以接下来我们就来看看如果把递归函数改成迭代的方式。
...
对比两次改写，一个关键的区别浮出水面：阶乘的改写只用了两个变量，树遍历的改写却必须自己维护一个栈。为什么？因为阶乘的尾递归版本本来就只有固定数量的状态；而中序遍历天生需要记住"处理完左子树之后，回来还要干什么"，这份记忆无论如何都得有地方放。递归时它藏在调用栈里，改成迭代它就显形为那个显式的 `stack`。换句话说，**改写并没有消除状态，只是把状态从隐式变成了显式**——这也解释了为什么这类改写总是"逻辑变复杂了"：复杂性没有增加，只是从语言的运行时搬到了你的代码里。
