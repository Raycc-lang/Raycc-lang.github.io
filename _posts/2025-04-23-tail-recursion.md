...
我们之前说过，在递归过程中运算和值会先保存在堆栈里：factorial(5) 挂起了 4 次待完成的乘法，factorial(10000) 就要挂起 9999 个栈帧——内存需求随递归深度线性增长，这就是问题所在。如果把这个递归函数写成尾递归的形式，在计算的过程中，状态会随着参数传递给下一层，运算结果不在需要用到之前的运算状态。编程语言实现时若对递归过程进行优化，使其直接丢弃前序运算而非压栈保存，可显著降低函数的内存需求。
