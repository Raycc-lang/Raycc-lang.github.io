...
该值(-62.63)即目标函数在当前点的瞬时变化率（曲线的斜率），几何上表示为曲线在该点的切线斜率。整个参数集（如w、b）对目标函数的瞬时变化率（是一个向量）叫做**梯度(gradient)** ，表示该参数集对目标函数的综合影响。
...
接下来，我们将上述辅助函数整合为统一的梯度下降接口。该接口应该设计为通用接口，可应用于任意模型（不限于线性模型），所以我们把目标函数，初始参数和学习率等作为参数。
```python
...
```
该函数内部是经过了两次再版的update函数，它是根据梯度下降这个接口函数的参数生成的。
...
当前遇到的问题是基于列表实现的函数无法处理高维数组，虽然可以暂时修改向量微分算子（$\nabla$函数）的实现来暂时解决这个问题，但是更好的办法是实现一个更通用的解决方案，而这正是下一篇文章的主题。我们将会从一个非常酷的概念——**张量（tensor）**入手，到彻底解决所有高维数组操作问题。

读完本文你已经了解了整个训练线性模型/学习参数的过程，现在先休息一下吧。