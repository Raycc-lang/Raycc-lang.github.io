...
你可能已经注意到了一个规律：标量→向量→矩阵，这个序列有什么共同特征吗？

现在请想象一个更通用的数据结构——张量。张量可以看作是标量、向量、矩阵的统一概念：
...
1阶张量：就是向量，如 `[1, 2, 3]`
2阶张量：就是矩阵，如 [`[1,2], [3,4]]`
3阶张量：可以想象成多个矩阵叠在一起，如一个 RGB 彩色图片。
更高阶张量：虽然难以在脑中可视化成空间维度，但概念是一样的。

注意：这里用空间维度来帮助理解，但是'阶'不是指空间维度。一个3阶张量不一定代表3D空间中的数据，它只是说需要3个索引来定位其中的元素。这个概念有的地方会翻译成秩，它和矩阵的秩又不是一个概念。它是一个很简单的概念，但是很容易混淆，在不用的场景在要小心运用。

在程序实现中，张量就是多维数组的抽象表示。
...
```
在新的ext1实现中，除了接受一个函数f作为参数，还有另一个参数代表参数函数f所操作的张量的阶r。现在ext1函数并不会一直递归到f处理的参数变成Scalar为止，而是到f的参数递归变化为阶数n为止。
...
以上代码逻辑可能会显得比较抽象，这里值得多花点时间，善用工具，比如代码编辑器的debug功能，多找几个例子帮助自己理解。
