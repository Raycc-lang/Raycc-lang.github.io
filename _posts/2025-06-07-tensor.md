...
title:  "(三)张量"
description: "《The Little Learner》系列第三篇，定义标量向量矩阵与张量及其阶和形状，实现张量运算并对比Numpy与JAX的实现"
...
比如：$[5.0 \ \ 7.18 \ \ π]$     $[2.0\ 1.0\ 4.0\ 3.0] $都是一阶张量，但是第一个内部有三个零阶张量，第二个内部有四个零阶张量。我们用Python里的元组符号也就是括号来表示张量的形状，因此这两个一阶张量的形状分别为（3,）和（4,）。
...
# Python的类型提示实在是一言难尽，Python3.8之后开始支持上面使用字符串的方式表示递归类型定义，但是在这样的场景中还是会报错，建议直接忽略。
...
# 形状同理，标量的形状用空元组表示
# 另外，这里只数了第一个元素的数量，也就是假设所有同阶元素子元素数量相同
...
接下来看看对于张量的操作。张量操作有很多，这里我把它们分为几类：

1. **逐元素二元操作（例如加法、乘法）**：将两个同形状张量对应位置的元素进行二元运算。
    >比如向量加法$[a, b] + [c, d] = [a + c, b + d]$ ；哈达玛积 $[a,b] ⊙ [c,d] = [a*c, b*d]$
2. **按元素映射**：将标量→标量函数映射到张量的每个元素。
    >又叫做向量化（vectorization）或元素级映射，比如之前实现过的sqr函数；再比如标量乘法$a × [b, c] = [a × b, a × c]$
3. **降维操作**：对张量沿最外层维度进行操作，结果张量的阶降低 1。
    >比如之前用到过的sum函数
4. 其他
    >上一篇文章中实现的点积等等
...
同理：`zeros`函数的参数是一个任意阶的张量，返回值是一个阶数和形状与参数相同，但是数值全是零的张量；`tsqr/tsqrt`返回值是一个阶数和形状与参数张量相同，但是所有最内部元素（也就是标量的值）是参数对应值的平方/平方根组成的张量；`sum_1`函数原本是处理一阶张量/数组的函数，被ext1拓展成tsum。tsum可以处理更高阶(高于1的任意阶)的张量，结果也是一个张量，不过最内部的一阶张量被加总成一个值所以结果张量比参数张量阶减少1; flatten原本是把一个二阶张量/矩阵变成一个一阶张量/数组的函数，现在能让所有高阶张量的阶数减少2。
...
以上代码逻辑可能会显得比较抽象，最好亲手追踪一次完整的调用。以 `tadd(2.0, [1.0, 2.0, 3.0])` 为例：

1. 进入 `ext2` 返回的 op：`of_ranks(0, t, 0, u)`？t 是标量（rank 0）✓，但 u 是向量（rank 1）✗——不满足，进入 `desc`；
2. `desc` 里第一个分支 `of_rank(0, t)` 命中：t 已经到达目标阶，于是把 t **固定**，对 u 的每个元素递归——`[g(2.0, 1.0), g(2.0, 2.0), g(2.0, 3.0)]`；
3. 这三次递归调用里两个参数都是标量了，`of_ranks` 满足，落到最底层的 `add_00`——得到 `[3.0, 4.0, 5.0]`。

走一遍就会发现“广播”只是“谁没到目标阶，就拆谁”的递归下降。建议再亲手追踪一次 `tadd([1.0, 2.0], [[1.0, 1.0], [2.0, 2.0]])`，这次会命中不同的分支。善用工具，比如代码编辑器的debug功能，对于思考高级张量操作非常有用。也可以对比其他实现，比如你可以发现我们实现的dot函数与numpy.dot相比，在向量操作上逻辑基本相同，但是在矩阵或者更高阶张量上的操作就很不一样了。
...
首先我们应该把所有的输入输出和参数都用张量来表示，并且使用上面的张量操作，而不是+、—等操作符号：
```python
...
同时请留意这段代码的开销：**每一个标量参数**都要做一次整个 theta 的 deepcopy，外加一次完整的前向计算。参数有 n 个标量，一次求梯度的代价就是 n 次深拷贝 + n+1 次前向——参数规模一大，这是平方级的浪费。我们暂时接受它，因为它够简单、能用；但这笔账会越欠越多，直到自动微分那一篇来彻底清算。
...
# ndarray是Numpy实现的张量对象;jax中是jax.Array
Tensor =  np.ndarray | float | int

def is_scalar(x) -> bool:
    return np.isscalar(x) 

# Numpy.array函数是创建ndarray对象的构造函数
def tensor(elements) -> Tensor:
    return np.array(elements)

# Numpy中的阶和形状是元数据，可以直接通过对象的ndim和shape属性获取
def rank(t: Tensor) -> int:
...
本文还拓展了Nabla函数，使其能计算任意张量的梯度，并解决了上文中遇到的问题。不过当前Nabla（梯度计算）函数实现依然是基于数值微分，这种实现方式不仅复杂而且还有一些重大的缺陷。这个缺陷我们可能会在下一篇博文中遭遇，不过不用担心，我也会很快介绍自动微分的实现方式。
