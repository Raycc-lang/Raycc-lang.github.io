...
欢迎来到本系列第三篇！如果说梯度下降是深度学习的优化引擎，那么今天将介绍的张量（Tensor）就是其燃料系统，它是所有深度学习框架的底层支柱。

这个概念将帮助我们：
统一数据表达：从图像、文本到音频，从标量到高维数据，都可以通过张量进行统一建模。
突破计算维度：张量支持任意维度的数据结构，使得模型能够灵活处理不同形态的数据输入。
奠基模型架构：神经网络的权重与输入的本质载体，都是张量。无论是卷积层、全连接层还是循环层，它们的操作本质上都是对张量的变换。（体会一下TensorFlow这个知名深度学习框架名的意思）。

虽然本文概念独立成篇，但后续关于自动微分与网络构建的内容，都离不开张量操作。强烈建议按顺序阅读以获取完整知识拼图。
...
zeros = ext1((lambda _: 0), 0)
tsqr = ext1((lambda x: x * x), 0)
tsqrt = ext1((lambda x: x ** 0.5), 0)
...
```
用`add1`这个函数举例理解：`r`这个参数反应被拓展函数原本操作的是具体哪一阶的张量，匿名函数`(lambda x: x + 1)`操作对象是标量，因此该函数的`r`值是0；处理高阶张量的时候是要该张量的元素不是标量，便会调用map函数，也就是把该张量的元素带入匿名函数中。

同理：`zero`函数返回一个阶数和形状相同，但是数值全是零的张；squ返回值是一个所有元素的平方根组成的张量；sum函数原本是处理一阶张量/数组的函数，现在可以处理更高阶的张量; flatten原本是把一个二阶张量/矩阵变成一个一阶张量/数组的函数，现在能让所有高阶张量的阶数减少1。
...
tdiv = ext2_broadcast((lambda m, n: m / n), 0, 0)
...
Tensor =  Union[np.ndarray, float, int]
...
def ext(func, *tensors: Tensor) -> Tensor:
    # 这里是高阶函数
...
```
这里之所以提JAX而不是更常见的Pytorch、TensorFlow等框架是因为JAX显式遵循函数式编程范式，很容易把之前学到东西转换到JAX上（除了JAX可以显式指定按照某个维度展开，比我们的实现要更强大之外，跟我们的实现效果差不多），甚至可以直接使用JAX.grad代替nabla函数而不用修改任何代码。
