...
为了解决这个问题，就需要消除列表中的负值。有两个方法可以做到：一个是取绝对值，一个是计算平方。两种方法得出来的结果分别叫做**绝对损失（Absolute Loss）**和**平方损失（squared error）**，也叫做**L1**或者**L2损失**。两种方式各有其适用场景，对于回归任务，L2损失更合适，这里只实现L2损失。
